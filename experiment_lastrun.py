...
image_size = (300, 300)
possible_positions = [(0,400), (0,-400), (400, 0), (-400, 0)]
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "baseline_introduction" ---
    baseline_text = visual.TextStim(win=win, name='baseline_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    baseline_intro_response = keyboard.Keyboard()
    ...
    baseline_circle1 = visual.ShapeStim(
        win=win, name='baseline_circle1',units='pix', 
        size=circle_size, vertices='circle',
        ori=0.0, pos=[0,0], anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-2.0, interpolate=True)
    baseline_circle2 = visual.ShapeStim(
        win=win, name='baseline_circle2',units='pix', 
        size=circle_size, vertices='circle',
        ori=0.0, pos=[0,0], anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor=color, fillColor=color,
        opacity=None, depth=-3.0, interpolate=True)
    baseline_circle3 = visual.ShapeStim(
        win=win, name='baseline_circle3',units='pix', 
        size=circle_size, vertices='circle',
        ori=0.0, pos=[0,0], anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-4.0, interpolate=True)
    baseline_target = visual.Rect(
        win=win, name='baseline_target',units='pix', 
        width=rectangle_size[0], height=rectangle_size[1],
        ori=0.0, pos=[0,0], anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-5.0, interpolate=True)
    baseline_response = event.Mouse(win=win)
    x, y = [None, None]
    baseline_response.mouseClock = core.Clock()
    ...
    action_fixation = visual.ShapeStim(
        ...
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-1.0, interpolate=True)
    action_circle1 = visual.ShapeStim(
        win=win, name='action_circle1',units='pix', 
        size=circle_size, vertices='circle',
        ori=0.0, pos=[0,0], anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-2.0, interpolate=True)
    action_circle2 = visual.ShapeStim(
        win=win, name='action_circle2',units='pix', 
        size=circle_size, vertices='circle',
        ori=0.0, pos=[0,0], anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor=color, fillColor=color,
        opacity=None, depth=-3.0, interpolate=True)
    ...
    # --- Prepare to start Routine "baseline_introduction" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('baseline_introduction.started', globalClock.getTime())
    baseline_intro_response.keys = []
    baseline_intro_response.rt = []
    _baseline_intro_response_allKeys = []
    # keep track of which components have finished
    baseline_introductionComponents = [baseline_text, baseline_intro_response]
    for thisComponent in baseline_introductionComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "baseline_introduction" ---
    routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        ...
        # *baseline_intro_response* updates
        waitOnFlip = False
        
        # if baseline_intro_response is starting this frame...
        if baseline_intro_response.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            baseline_intro_response.frameNStart = frameN  # exact frame index
            baseline_intro_response.tStart = t  # local t and not account for scr refresh
            baseline_intro_response.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(baseline_intro_response, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'baseline_intro_response.started')
            # update status
            baseline_intro_response.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(baseline_intro_response.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(baseline_intro_response.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if baseline_intro_response.status == STARTED and not waitOnFlip:
            theseKeys = baseline_intro_response.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _baseline_intro_response_allKeys.extend(theseKeys)
            if len(_baseline_intro_response_allKeys):
                baseline_intro_response.keys = _baseline_intro_response_allKeys[0].name  # just the first key pressed
                baseline_intro_response.rt = _baseline_intro_response_allKeys[0].rt
                baseline_intro_response.duration = _baseline_intro_response_allKeys[0].duration
                # a response ends the routine
                continueRoutine = False
    ...
    # --- Ending Routine "baseline_introduction" ---
    for thisComponent in baseline_introductionComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('baseline_introduction.stopped', globalClock.getTime())
    # check responses
    if baseline_intro_response.keys in ['', [], None]:  # No response was made
        baseline_intro_response.keys = None
    thisExp.addData('baseline_intro_response.keys',baseline_intro_response.keys)
    if baseline_intro_response.keys != None:  # we had a response
        thisExp.addData('baseline_intro_response.rt', baseline_intro_response.rt)
        thisExp.addData('baseline_intro_response.duration', baseline_intro_response.duration)
    thisExp.nextEntry()
    # the Routine "baseline_introduction" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    baseline_loop = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='baseline_loop')
    thisExp.addLoop(baseline_loop)  # add the loop to the experiment
    thisBaseline_loop = baseline_loop.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisBaseline_loop.rgb)
    if thisBaseline_loop != None:
        for paramName in thisBaseline_loop:
            globals()[paramName] = thisBaseline_loop[paramName]
    
    for thisBaseline_loop in baseline_loop:
        currentLoop = baseline_loop
        ...
        baseline_circle1.setPos(pos_baseline_circle1)
        baseline_circle2.setPos(pos_baseline_circle2)
        baseline_circle3.setPos(pos_baseline_circle3)
        baseline_target.setPos(pos_baseline_target)
        # setup some python lists for storing info about the baseline_response
        baseline_response.x = []
        baseline_response.y = []
        baseline_response.leftButton = []
        baseline_response.midButton = []
        baseline_response.rightButton = []
        baseline_response.time = []
        baseline_response.clicked_name = []
        gotValidClick = False  # until a click is received
        # keep track of which components have finished
        baselineComponents = [fixation, baseline_circle1, baseline_circle2, baseline_circle3, baseline_target, baseline_response]
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # *baseline_circle1* updates
            
            # if baseline_circle1 is starting this frame...
            if baseline_circle1.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                baseline_circle1.frameNStart = frameN  # exact frame index
                baseline_circle1.tStart = t  # local t and not account for scr refresh
                baseline_circle1.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(baseline_circle1, 'tStartRefresh')  # time at next scr refresh
                # update status
                baseline_circle1.status = STARTED
                baseline_circle1.setAutoDraw(True)
            
            # if baseline_circle1 is active this frame...
            if baseline_circle1.status == STARTED:
                # update params
                pass
            
            # if baseline_circle1 is stopping this frame...
            if baseline_circle1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > baseline_circle1.tStartRefresh + 8.0-frameTolerance:
                    # keep track of stop time/frame for later
                    baseline_circle1.tStop = t  # not accounting for scr refresh
                    baseline_circle1.frameNStop = frameN  # exact frame index
                    # update status
                    baseline_circle1.status = FINISHED
                    baseline_circle1.setAutoDraw(False)
            
            # *baseline_circle2* updates
            
            # if baseline_circle2 is starting this frame...
            if baseline_circle2.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                baseline_circle2.frameNStart = frameN  # exact frame index
                baseline_circle2.tStart = t  # local t and not account for scr refresh
                baseline_circle2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(baseline_circle2, 'tStartRefresh')  # time at next scr refresh
                # update status
                baseline_circle2.status = STARTED
                baseline_circle2.setAutoDraw(True)
            
            # if baseline_circle2 is active this frame...
            if baseline_circle2.status == STARTED:
                # update params
                pass
            
            # if baseline_circle2 is stopping this frame...
            if baseline_circle2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > baseline_circle2.tStartRefresh + 8.0-frameTolerance:
                    # keep track of stop time/frame for later
                    baseline_circle2.tStop = t  # not accounting for scr refresh
                    baseline_circle2.frameNStop = frameN  # exact frame index
                    # update status
                    baseline_circle2.status = FINISHED
                    baseline_circle2.setAutoDraw(False)
            
            # *baseline_circle3* updates
            
            # if baseline_circle3 is starting this frame...
            if baseline_circle3.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                baseline_circle3.frameNStart = frameN  # exact frame index
                baseline_circle3.tStart = t  # local t and not account for scr refresh
                baseline_circle3.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(baseline_circle3, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'baseline_circle3.started')
                # update status
                baseline_circle3.status = STARTED
                baseline_circle3.setAutoDraw(True)
            
            # if baseline_circle3 is active this frame...
            if baseline_circle3.status == STARTED:
                # update params
                pass
            
            # if baseline_circle3 is stopping this frame...
            if baseline_circle3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > baseline_circle3.tStartRefresh + 8.0-frameTolerance:
                    # keep track of stop time/frame for later
                    baseline_circle3.tStop = t  # not accounting for scr refresh
                    baseline_circle3.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'baseline_circle3.stopped')
                    # update status
                    baseline_circle3.status = FINISHED
                    baseline_circle3.setAutoDraw(False)
            
            # *baseline_target* updates
            
            # if baseline_target is starting this frame...
            if baseline_target.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'baseline_target.started')
                # update status
                baseline_target.status = STARTED
                baseline_target.setAutoDraw(True)
            
            # if baseline_target is active this frame...
            if baseline_target.status == STARTED:
                # update params
                pass
            
            # if baseline_target is stopping this frame...
            if baseline_target.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > baseline_target.tStartRefresh + 8.0-frameTolerance:
                    # keep track of stop time/frame for later
                    baseline_target.tStop = t  # not accounting for scr refresh
                    baseline_target.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'baseline_target.stopped')
                    # update status
                    baseline_target.status = FINISHED
                    baseline_target.setAutoDraw(False)
            # *baseline_response* updates
            
            # if baseline_response is starting this frame...
            if baseline_response.status == NOT_STARTED and t >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                baseline_response.frameNStart = frameN  # exact frame index
                baseline_response.tStart = t  # local t and not account for scr refresh
                baseline_response.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(baseline_response, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.addData('baseline_response.started', t)
                # update status
                baseline_response.status = STARTED
                baseline_response.mouseClock.reset()
                prevButtonState = baseline_response.getPressed()  # if button is down already this ISN'T a new click
            
            # if baseline_response is stopping this frame...
            if baseline_response.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > baseline_response.tStartRefresh + 8.0-frameTolerance:
                    # keep track of stop time/frame for later
                    baseline_response.tStop = t  # not accounting for scr refresh
                    baseline_response.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.addData('baseline_response.stopped', t)
                    # update status
                    baseline_response.status = FINISHED
            if baseline_response.status == STARTED:  # only update if started and not finished!
                buttons = baseline_response.getPressed()
                if buttons != prevButtonState:  # button state changed?
                    prevButtonState = buttons
                    if sum(buttons) > 0:  # state changed to a new click
                        # check if the mouse was inside our 'clickable' objects
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames(baseline_target, namespace=locals())
                        for obj in clickableList:
                            # is this object clicked on?
                            if obj.contains(baseline_response):
                                gotValidClick = True
                                baseline_response.clicked_name.append(obj.name)
                        x, y = baseline_response.getPos()
                        baseline_response.x.append(x)
                        baseline_response.y.append(y)
                        buttons = baseline_response.getPressed()
                        baseline_response.leftButton.append(buttons[0])
                        baseline_response.midButton.append(buttons[1])
                        baseline_response.rightButton.append(buttons[2])
                        baseline_response.time.append(baseline_response.mouseClock.getTime())
                        if gotValidClick:
        ...
        # store data for baseline_loop (TrialHandler)
        baseline_loop.addData('baseline_response.x', baseline_response.x)
        baseline_loop.addData('baseline_response.y', baseline_response.y)
        baseline_loop.addData('baseline_response.leftButton', baseline_response.leftButton)
        baseline_loop.addData('baseline_response.midButton', baseline_response.midButton)
        baseline_loop.addData('baseline_response.rightButton', baseline_response.rightButton)
        baseline_loop.addData('baseline_response.time', baseline_response.time)
        baseline_loop.addData('baseline_response.clicked_name', baseline_response.clicked_name)
    ...
    # completed 1.0 repeats of 'baseline_loop'
    ...
    action_loop = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('distractors.csv', selection=choice(268, size = 1, replace = False)),
    ...
    for thisAction_loop in action_loop:
        ...
        action_circle1.setPos(pos_action_circle1)
        action_circle2.setPos(pos_action_circle2)
        action_circle3.setPos(pos_action_circle3)
        action_target.setPos(pos_action_target)
        ...
        # keep track of which components have finished
        actionComponents = [action_fixation, action_circle1, action_circle2, action_circle3, action_target, action_response, action_distractor]
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # *action_circle1* updates
            
            # if action_circle1 is starting this frame...
            if action_circle1.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                action_circle1.frameNStart = frameN  # exact frame index
                action_circle1.tStart = t  # local t and not account for scr refresh
                action_circle1.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(action_circle1, 'tStartRefresh')  # time at next scr refresh
                # update status
                action_circle1.status = STARTED
                action_circle1.setAutoDraw(True)
            
            # if action_circle1 is active this frame...
            if action_circle1.status == STARTED:
                # update params
                pass
            
            # if action_circle1 is stopping this frame...
            if action_circle1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > action_circle1.tStartRefresh + 8.0-frameTolerance:
                    # keep track of stop time/frame for later
                    action_circle1.tStop = t  # not accounting for scr refresh
                    action_circle1.frameNStop = frameN  # exact frame index
                    # update status
                    action_circle1.status = FINISHED
                    action_circle1.setAutoDraw(False)
            
            # *action_circle2* updates
            
            # if action_circle2 is starting this frame...
            if action_circle2.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                action_circle2.frameNStart = frameN  # exact frame index
                action_circle2.tStart = t  # local t and not account for scr refresh
                action_circle2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(action_circle2, 'tStartRefresh')  # time at next scr refresh
                # update status
                action_circle2.status = STARTED
                action_circle2.setAutoDraw(True)
            
            # if action_circle2 is active this frame...
            if action_circle2.status == STARTED:
                # update params
                pass
            
            # if action_circle2 is stopping this frame...
            if action_circle2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > action_circle2.tStartRefresh + 8.0-frameTolerance:
                    # keep track of stop time/frame for later
                    action_circle2.tStop = t  # not accounting for scr refresh
                    action_circle2.frameNStop = frameN  # exact frame index
                    # update status
                    action_circle2.status = FINISHED
                    action_circle2.setAutoDraw(False)
            
            # *action_circle3* updates
            
            # if action_circle3 is starting this frame...
            if action_circle3.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                action_circle3.frameNStart = frameN  # exact frame index
                action_circle3.tStart = t  # local t and not account for scr refresh
                action_circle3.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(action_circle3, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'action_circle3.started')
                # update status
                action_circle3.status = STARTED
                action_circle3.setAutoDraw(True)
            
            # if action_circle3 is active this frame...
            if action_circle3.status == STARTED:
                # update params
                pass
            
            # if action_circle3 is stopping this frame...
            if action_circle3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > action_circle3.tStartRefresh + 8.0-frameTolerance:
                    # keep track of stop time/frame for later
                    action_circle3.tStop = t  # not accounting for scr refresh
                    action_circle3.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'action_circle3.stopped')
                    # update status
                    action_circle3.status = FINISHED
                    action_circle3.setAutoDraw(False)
            
            # *action_target* updates
            
            # if action_target is starting this frame...
            if action_target.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                action_target.frameNStart = frameN  # exact frame index
                action_target.tStart = t  # local t and not account for scr refresh
                action_target.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(action_target, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'action_target.started')
                # update status
                action_target.status = STARTED
                action_target.setAutoDraw(True)
            
            # if action_target is active this frame...
            if action_target.status == STARTED:
                # update params
                pass
            
            # if action_target is stopping this frame...
            if action_target.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > action_target.tStartRefresh + 8.0-frameTolerance:
                    # keep track of stop time/frame for later
                    action_target.tStop = t  # not accounting for scr refresh
                    action_target.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'action_target.stopped')
                    # update status
                    action_target.status = FINISHED
                    action_target.setAutoDraw(False)
            # *action_response* updates
            
            # if action_response is starting this frame...
            if action_response.status == NOT_STARTED and t >= 2.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if action_response.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        # check if the mouse was inside our 'clickable' objects
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames(action_target, namespace=locals())
                        for obj in clickableList:
                            # is this object clicked on?
            ...
            if action_distractor.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
